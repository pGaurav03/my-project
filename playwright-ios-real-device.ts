...
(async () => {
  ...
  try {
    ...
    await page.evaluate((_: any) => {}, `lambdatest_action: ${JSON.stringify({ action: "setTestStatus", arguments: {status: "passed", remark: "Wikipedia test passed - found 3 occurrences of '19th century'" },})}`);
    ...
    await page.evaluate((_: any) => {}, `lambdatest_action: ${JSON.stringify({action: "setTestStatus", arguments: { status: "failed", remark: e.message }})}`);
...
async function teardown(page: any, context: any, browser: any) {
